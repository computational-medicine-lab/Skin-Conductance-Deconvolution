...
%                 subplot(3,1,1); stem(x(:,k)); title('x_k');
...
%         -- End Display --
        ...
        if(index_xiter > xiter/2  &  num > targetdiv  &  targetdiv > 0)
            x(x(:,k)<amp_threshold,k)=0;
            nonzero_idxs = find(x(:,k)>0);
            ll = length(nonzero_idxs);
            for ii = 1:ll % restricting multiple impulses in 11 min window
                if(nonzero_idxs(ii)<separation+1)
                    range = 1:nonzero_idxs(ii)+separation;
                    [val, ~] = max( x(range,k));
                    if(val ~= x(nonzero_idxs(ii),k)) x(nonzero_idxs(ii),k) = 0.0;end
                
                elseif(nonzero_idxs(ii)>n-separation)
                    range = (nonzero_idxs(ii)-separation):n;
                    [val, ~] = max( x(range,k));
                    if(val ~= x(nonzero_idxs(ii),k)) x(nonzero_idxs(ii),k) = 0.0;end
                    
                else
                    range = (nonzero_idxs(ii)-separation):(nonzero_idxs(ii)+separation);
                    [~, idx] = max( x(range,k));
                    if(idx ~= separation+1) x(nonzero_idxs(ii),k) = 0.0;end
                end
            end
            x(:,k) = pickhighest(x(:,k), targetdiv)';
        end
        
        if(index_xiter > xiter/2)
            x(x(:,k)<amp_threshold,k)=0;
            nonzero_idxs = find(x(:,k)>0);
            ll = length(nonzero_idxs);
            for ii = 1:ll % restricting multiple impulses in 11 min window
